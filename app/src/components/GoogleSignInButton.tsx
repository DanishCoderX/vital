...
// You need separate OAuth client IDs per platform — see README.md for the full walkthrough.
...
export default function GoogleSignInButton({ onIdToken, label = "Continue with Google" }: GoogleSignInButtonProps) {
  ...
  if (!configured) {
    return (
      <TouchableOpacity
        onPress={() => Alert.alert("Not configured", "Google sign-in needs OAuth client IDs — see README.md.")}
